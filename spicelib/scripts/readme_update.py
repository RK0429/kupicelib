...
while line_no < len(readme_md):
    ...
    if "```python" in line:
        ...
        if line_no + 1 < len(readme_md):
            ...
            if not m and line_no + 2 < len(
                readme_md
            ):  # if there is no -- in statement, check the next line
                ...
                if (loc := m.group("loc")) is not None:
                    ...
                    for line1 in include_text:
                        if start_tag in line1:
                            include_ident = line1.find(
                                "#"
                            )  # This marks the indent of all the block
                        elif end_tag in line1:
                            include_ident = -1
                        else:
                            if (
                                include_ident >= 0
                            ):  # means that we are reading between the Start and End
