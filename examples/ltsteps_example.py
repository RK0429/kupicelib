...
# Printing Headers
print(
    " ".join([f"{step:15s}" for step in step_names]), end=""
)  # Print steps names with no new line
print(" ".join([f"{name:15s}" for name in meas_names]), end="\n")
# Printing data
for i in range(data.step_count):
    print(
        " ".join([f"{data[step][i]:15}" for step in step_names]), end=""
    )  # Print steps names with no new line
    print(
        " ".join([f"{data[name][i]:15}" for name in meas_names]), end="\n"
    )  # Print Header
