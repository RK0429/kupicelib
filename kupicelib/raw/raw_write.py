...
class Trace(DataSet):
    ...
    This class extends DataSet and represents a single trace or signal in a RAW file. It
    doesn't support STEPPED data as it's designed for write operations.

    Args:     name (str): Name of the trace being created     data
    (Union[Sequence[Union[float, complex]], ndarray]): Data points for the trace
    whattype (str, optional): Type of trace data - "time", "frequency", "voltage" or
    "current". Defaults to "voltage".     numerical_type (str, optional): Numerical
    representation - "real", "complex", or "double". If empty, will be inferred from the
    data. Defaults to "".

    Raises:     AssertionError: If the trace is a frequency trace but no numerical_type
    is specified     NotImplementedError: If the data type cannot be inferred

    Note:     The first trace in a RAW file is typically an axis trace (time or
    frequency).     For frequency traces, numerical_type must be explicitly specified
    ('complex' for AC analysis or     'double' for NOISE analysis).
    """

    def __init__(
        self,
        name: str,
        data: Sequence[float | complex] | ndarray,
...
class RawWrite:
    ...
    Args:     plot_name (Optional[str], optional): Name of the plot. If None, will be
    ...
    def add_trace(self, trace: Trace) -> None:
        ...
        The trace must have the same size as trace 0 ('time', 'frequency', etc.). The
        first trace added defines the X-Axis and the type of RAW file being generated.
        If no plot name was defined, it will automatically assign a name based on the
        first trace.

        Args:     trace (Trace): The trace to add. Must be a Trace instance.

        Raises:     AssertionError: If trace is not a Trace instance     ValueError: If
        ...
        if len(self._traces) == 0:
            if trace.whattype == "time":
                ...
                raise ValueError(
                    "First trace must be one of 'time', 'frequency', 'param', 'voltage', or '...'"
                )
    ...
    def save(self, filename: str | Path) -> None:
        ...
        with open(filename, "wb") as f:
            ...
            f.write(
                f"No. Points: {len(self._traces[0]):12}\n".encode(self.encoding)
            )
            ...
            if (
                ...
                fmts: dict[Trace, Callable[[_SupportsToBytes], bytes]] = {
                    trace: tobytes_for_trace(trace) for trace in self._traces
                }
    ...
    @staticmethod
    def _rename_netlabel(name: str, **kwargs: object) -> str:
        """Rename a trace name while preserving V() or I() containers.

        Args:     name (str): The original trace name     **kwargs: Keyword arguments
        including potential rename_format

        Returns:     str: The renamed trace name

        Note:     If rename_format is provided in kwargs, it will be used to format the
        name.     For voltage/current signals (V(...) or I(...)), only the content
    ...
    def _name_exists(self, name: str) -> bool:
        ...
        Args:     name (str): The trace name to check
    ...
    def add_traces_from_raw(
        ...
        **kwargs: object,
        ...
        admissible_error = float(
            cast(float | int | str, options.get("admissible_error", 1e-11))
        )
        from_step = int(cast(int | str, options.get("step", 0)))
        minimum_timestep = float(
            cast(float | int | str, options.get("minimum_timestep", 0.0))
        )
        ...
        other_flags = cast(str, other.get_raw_property("Flags")).split(" ")
        ...
        if len(self._traces):  # there are already traces
            ...
            self.plot_name = cast(str, other.get_raw_property("Plotname"))
        ...
        if force_axis_alignment or minimum_timestep > 0.0:
            my_axis = (
                self._new_axis or self._traces[0].get_wave().tolist()
            )
    ...
    def _interpolate(
        ...
        Args:     trace_data (ndarray): The original data values     trace_axis
        (ndarray): The original axis values     new_axis (Sequence[float]): The new axis
        values to interpolate to

        Returns:     ndarray: Interpolated data aligned with the new axis

        Note:     Uses linear interpolation between data points.
        """
    ...
    def _consolidate(self) -> None:
        ...
        This internal method is called before saving to ensure all traces are aligned to
        the same axis. Traces are interpolated as needed to match the new axis.
        """
    ...
    def get_trace(self, trace_ref: str | int) -> Trace:
        ...
        Args:     trace_ref (Union[str, int]): Either the trace name as string or the
        trace index

        Returns:     Trace: The requested trace

        Raises:     IndexError: If the trace doesn't exist
        """
    ...
    def __getitem__(self, item: str | int) -> Trace:
        """Access traces using the [ ] operator.

        Args:     item (Union[str, int]): Either the trace name or index
...
def tobytes_for_trace(trace: Trace) -> Callable[[_SupportsToBytes], bytes]:
    ...
    values from a specific trace to bytes.

    Args:     trace (Trace): The trace for which to create a conversion function

    Returns:     Callable[[Any], bytes]: A function that converts a value to bytes
    """

    def tobytes(value: _SupportsToBytes) -> bytes:
