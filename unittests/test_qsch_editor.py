...
def equalFiles(
    ...
    testcase.assertEqual(
        len(lines1),
        len(lines2),
        f'Files "{file1}" and "{file2}" have different number of lines',
    )
    for i in range(len(lines1)):
        ...
        testcase.assertEqual(
            data1, data2, f'Files "{file1}" and "{file2}" are not equal'
        )
...
class ASC_Editor_Test(unittest.TestCase):
    ...
    def setUp(self):
        self.edt = kupicelib.editor.qsch_editor.QschEditor(test_dir + "DC sweep.qsch")
    ...
    def test_component_editing(self):
        ...
        equalFiles(
            self,
            temp_dir + "test_components_output.qsch",
            golden_dir + "test_components_output.qsch",
        )
        self.assertEqual(
        ...
        equalFiles(
            self,
            temp_dir + "test_components_output_2.qsch",
            golden_dir + "test_components_output_2.qsch",
        )
        ...
        equalFiles(
            self,
            temp_dir + "test_components_output_1.qsch",
            golden_dir + "test_components_output_1.qsch",
        )
    ...
    def test_component_editing_obj(self):
        ...
        equalFiles(
            self,
            temp_dir + "test_components_output_obj.qsch",
            golden_dir + "test_components_output_obj.qsch",
        )
        self.assertEqual(
            self.edt.get_component_value("R1"), "33k", "Tested R1 Value"
        )  # add assertion here
        ...
        equalFiles(
            self,
            temp_dir + "test_components_output_2_obj.qsch",
            golden_dir + "test_components_output_2_obj.qsch",
        )
        r1_params = r1.params
        for key, value in {"Tc1": "0", "Tc2": "0"}.items():
            self.assertEqual(r1_params[key], value, f"Tested R1 {key} Parameter")
    ...
    def test_parameter_edit(self):
        ...
        equalFiles(
            self,
            temp_dir + "test_parameter_output.qsch",
            golden_dir + "test_parameter_output.qsch",
        )
    ...
    def test_instructions(self):
        ...
        equalFiles(
            self,
            temp_dir + "test_instructions_output.qsch",
            golden_dir + "test_instructions_output.qsch",
        )
        self.edt.remove_instruction(".save I(R1)")
        self.edt.save_netlist(temp_dir + "test_instructions_output_1.qsch")
        equalFiles(
            self,
            temp_dir + "test_instructions_output_1.qsch",
            golden_dir + "test_instructions_output_1.qsch",
        )
        ...
        equalFiles(
            self,
            temp_dir + "test_instructions_output_2.qsch",
            golden_dir + "test_instructions_output_2.qsch",
        )
...
class QschEditorRotation(unittest.TestCase):
    ...
    def test_component_rotations(self):
        self.edt = kupicelib.editor.qsch_editor.QschEditor(
            test_dir + "qsch_rotation.qsch"
        )
        self.edt.save_netlist(temp_dir + "qsch_rotation.net")
        equalFiles(
            self, temp_dir + "qsch_rotation.net", golden_dir + "qsch_rotation.net"
        )
        self.qsch = kupicelib.editor.qsch_editor.QschEditor(
            test_dir + "qsch_rotation_set_test.qsch"
        )
        for rotation in range(0, 360, 45):
            ...
            equalFiles(
                self,
                temp_dir + f"qsch_rotation_set_{rotation}.qsch",
                golden_dir + f"qsch_rotation_set_{rotation}.qsch",
            )
...
class QschEditorSpiceGeneration(unittest.TestCase):
    ...
    def test_hierarchical(self):
        self.edt = kupicelib.editor.qsch_editor.QschEditor(
            test_dir + "top_circuit.qsch"
        )
        self.edt.save_netlist(temp_dir + "top_circuit.net")
        if sys.platform.startswith("win"):
            equalFiles(
                self, temp_dir + "top_circuit.net", golden_dir + "top_circuit_win32.net"
            )
        else:
            equalFiles(
                self, temp_dir + "top_circuit.net", golden_dir + "top_circuit.net"
            )

    def test_all_elements(self):
        self.edt = kupicelib.editor.qsch_editor.QschEditor(
            test_dir + "all_elements.qsch"
        )
...
class QschEditorFromAscConversion(unittest.TestCase):
    ...
    def test_asc_to_qsch(self):
        from kupicelib.scripts.asc_to_qsch import convert_asc_to_qsch

        convert_asc_to_qsch(
            test_dir + "DC sweep.asc", temp_dir + "DC_sweep_from_asc.qsch"
        )
        equalFiles(
            self,
            temp_dir + "DC_sweep_from_asc.qsch",
            golden_dir + "DC_sweep_from_asc.qsch",
        )
...
class QschEditorFloatingNet(unittest.TestCase):
    ...
    def test_floating_net(self):
        self.edt = kupicelib.editor.qsch_editor.QschEditor(
            test_dir + "Qspice_bug_floating_net.qsch"
        )
        x1 = self.edt["X1"]
        ...
        equalFiles(
            self,
            temp_dir + "qsch_floating_net.net",
            golden_dir + "qsch_floating_net.net",
        )
...
class QschEditorEmbeddedSubckt(unittest.TestCase):
    ...
    def test_embedded_subckt(self):
        self.edt = kupicelib.editor.qsch_editor.QschEditor(
            test_dir + "Qspice_bug_embedded_subckt.qsch"
        )
        self.edt.save_netlist(temp_dir + "qsch_embedded_subckt.net")
        equalFiles(
            self,
            temp_dir + "qsch_embedded_subckt.net",
            golden_dir + "qsch_embedded_subckt.net",
        )

    def test_sub_circuit(self):
        self.edt = kupicelib.editor.qsch_editor.QschEditor(test_dir + "Qspice_top.qsch")
        # get sub-circuit
        ...
        equalFiles(
            self, golden_dir + "Qspice_top_edit.net", temp_dir + "Qspice_top_edit.net"
        )
