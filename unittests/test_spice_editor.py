...
class SpiceEditor_Test(unittest.TestCase):
    ...
    def setUp(self):
        self.edt = kupicelib.editor.spice_editor.SpiceEditor(test_dir + "DC sweep.net")
        self.edt2 = kupicelib.editor.spice_editor.SpiceEditor(
            test_dir + "opamptest.net"
        )
        self.edt3 = kupicelib.editor.spice_editor.SpiceEditor(
            test_dir + "/amp3/amp3.net"
        )
    ...
    def test_component_editing_1(self):
        ...
        self.equalFiles(
            temp_dir + "test_components_output.net",
            golden_dir + "test_components_output.net",
        )
        self.assertEqual(
            self.edt.get_component_value("R1"), "33k", "Tested R1 Value"
        )  # add assertion here
        self.edt.set_component_parameters("R1", Tc1=0, Tc2=0, pwr=None)
        self.edt.save_netlist(temp_dir + "test_components_output_2.net")
        self.equalFiles(
            temp_dir + "test_components_output_2.net",
            golden_dir + "test_components_output_2.net",
        )
        ...
        self.equalFiles(
            temp_dir + "test_components_output_1.net",
            golden_dir + "test_components_output_1.net",
        )
    ...
    def test_component_editing_2(self):
        ...
        self.assertEqual(
            self.edt2.get_component_value("V3"),
            "PWL(1u 0 +2n 1 +1m 1 +2n 0 +1m 0 +2n -1 +1m -1 +2n 0) AC 1",
            "Tested V3 Value",
        )  # complex value, with parameters
        self.assertEqual(
            self.edt2.get_component_value("XU1"), "level2", "Tested U1 Value"
        )  # has parameters
        self.assertEqual(
            self.edt2.get_component_parameters("XU1")["Rin"],
            "501Meg",
            "Tested U1 Rin Value",
        )  # last in the list
        self.assertEqual(
            self.edt2.get_component_value("XU2"), "AD549", "Tested U2 Value"
        )  # no parameters
        self.assertListEqual(
            self.edt2.get_components(),
            ["V1", "V2", "V3", "XU1", "XU2"],
            "Tested get_components",
        )
        ...
        self.equalFiles(
            temp_dir + "opamptest_output_1.net", golden_dir + "opamptest_output_1.net"
        )

    def test_parameter_edit(self):
        self.assertEqual(self.edt.get_all_parameter_names(), ["RES", "TEMP"])
        self.assertEqual(
            self.edt.get_parameter("TEMP"), "0", "Tested TEMP Parameter"
        )  # add assertion here
        self.edt.set_parameter("TEMP", 25)
        self.assertEqual(
            self.edt.get_parameter("TEMP"), "25", "Tested TEMP Parameter"
        )  # add assertion here
        self.edt.save_netlist(temp_dir + "test_parameter_output.net")
        self.equalFiles(
            temp_dir + "test_parameter_output.net",
            golden_dir + "test_parameter_output.net",
        )
        ...
        self.equalFiles(
            temp_dir + "test_parameter_output_1.net",
            golden_dir + "test_parameter_output_1.net",
        )
    ...
    def test_instructions(self):
        ...
        self.equalFiles(
            temp_dir + "test_instructions_output.net",
            golden_dir + "test_instructions_output.net",
        )
        self.edt.remove_instruction(".save I(R1)")
        self.edt.save_netlist(temp_dir + "test_instructions_output_1.net")
        self.equalFiles(
            temp_dir + "test_instructions_output_1.net",
            golden_dir + "test_instructions_output_1.net",
        )
        self.edt.remove_Xinstruction(
            r"\.save\sI\(.*\)"
        )  # removes all .save instructions for currents
        self.edt.save_netlist(temp_dir + "test_instructions_output_2.net")
        self.equalFiles(
            temp_dir + "test_instructions_output_2.net",
            golden_dir + "test_instructions_output_2.net",
        )
    ...
    def test_inductors(self):
        ...
        self.assertEqual(
            regex_l.match("LBCD N1 N2 {param*2+3}").group("nodes"),
            " N1 N2",
            "Tested Inductor Value",
        )
    ...
    def test_diodes(self):
        ...
        self.assertEqual(
            "1N4148",
            regex_d.match("D1 N1 N2 1N4148").group("value"),
            "Tested Diode Value",
        )
        self.assertEqual(
            "D", regex_d.match("D1 N1 N2 D").group("value"), "Tested Diode Model"
        )
    ...
    def test_bipolar(self):
        ...
        self.assertEqual(
            "2N3904",
            regex_q.match("Q1 N1 N2 N3 2N3904").group("value"),
            "Tested Transistor Value",
        )
        self.assertEqual(
            "Q",
            regex_q.match("Q1 N1 N2 N3 Q").group("value"),
            "Tested Transistor Model",
        )
    ...
    def test_mosfets(self):
        ...
        self.assertEqual(
            "IRF540",
            regex_m.match("M1 N1 N2 N3 N4 IRF540").group("value"),
            "Tested MOSFET Value",
        )
        self.assertEqual(
            "IRF540",
            regex_m.match("M1 N1 N2 N3 IRF540").group("value"),
            "Tested MOSFET Value",
        )
        self.assertEqual(
            "M", regex_m.match("M1 N1 N2 N3 N4 M").group("value"), "Tested MOSFET Model"
        )
    ...
    def test_subcircuits(self):
        ...
        self.assertEqual(
            "SUB1",
            regex_x.match("X12 N1 N2 N3 SUB1").group("value"),
            "Tested Subcircuit Value",
        )
        self.assertEqual(
            "SUB1",
            regex_x.match("X12 N1 N2 N3 N4 SUB1 x=123 y=4u").group("value"),
            "Tested Subcircuit Value",
        )
        self.assertEqual(
            " x=123 y=4u",
            regex_x.match("X12 N1 N2 N3 N4 SUB1 x=123 y=4u").group("params"),
            "Tested Subcircuit Parameters",
        )
        self.assertEqual(
            " N1 N2 N3 N4",
            regex_x.match("X12 N1 N2 N3 N4 SUB1 x=123 y=4u").group("nodes"),
            "Tested Subcircuit Ports",
        )
    ...
    def test_independent_sources(self):
        ...
        self.assertEqual(
            "2 AC 1",
            regex_v.match("V1 NC_08 NC_09 2 AC 1 Rser=3").group("value"),
            "Tested Voltage Source Value",
        )
        self.assertEqual(
            " Rser=3",
            regex_v.match("V1 NC_08 NC_09 2 AC 1 Rser=3").group("params"),
            "Tested Voltage Source Value",
        )
        self.assertEqual(
            "PWL(1u 0 +2n 1 +1m 1 +2n 0 +1m 0 +2n -1 +1m -1 +2n 0)",
            regex_v.match(
                "V1 N1 N2 PWL(1u 0 +2n 1 +1m 1 +2n 0 +1m 0 +2n -1 +1m -1 +2n 0) Rser=3 Cpar=4"
            ).group("value"),
            "Tested Voltage Source Value",
        )
        self.assertEqual(
            regex_v.match("V1 N001 0 5").group("value"),
            "5",
            "Tested Voltage Source Value",
        )
        self.assertEqual(
            regex_v.match("V1 N1 N2 5").group("value"),
            "5",
            "Tested Voltage Source Value",
        )
        self.assertEqual(
            regex_v.match("V1 N1 N2 5V").group("value"),
            "5V",
            "Tested Voltage Source Value",
        )
        self.assertEqual(
            regex_v.match("V1 N1 N2 {param}").group("value"),
            "{param}",
            "Tested Voltage Source Value",
        )
        ...
        self.assertEqual(
            "2 AC 1",
            regex_i.match("I1 NC_08 NC_09 2 AC 1 Rser=3").group("value"),
            "Tested Independent Current Source Value",
        )
        self.assertEqual(
            " Rser=3",
            regex_i.match("I1 NC_08 NC_09 2 AC 1 Rser=3").group("params"),
            "Tested Independent Current Source Value",
        )
        self.assertEqual(
            "PWL(1u 0 +2n 1 +1m 1 +2n 0 +1m 0 +2n -1 +1m -1 +2n 0)",
            regex_i.match(
                "I1 N1 N2 PWL(1u 0 +2n 1 +1m 1 +2n 0 +1m 0 +2n -1 +1m -1 +2n 0) Rser=3 Cpar=4"
            ).group("value"),
            "Tested Voltage Source Value",
        )
        self.assertEqual(
            regex_i.match("I1 N001 0 5").group("value"),
            "5",
            "Tested Independent Current Source Value",
        )
        self.assertEqual(
            regex_i.match("I1 N1 N2 5").group("value"),
            "5",
            "Tested Independent Current Source Value",
        )
        self.assertEqual(
            regex_i.match("I1 N1 N2 5V").group("value"),
            "5V",
            "Tested Independent Current Source Value",
        )
        self.assertEqual(
            regex_i.match("I1 N1 N2 {param}").group("value"),
            "{param}",
            "Tested Independent Current Source Value",
        )
    ...
    def test_subcircuits_edit(self):
        ...
        * Check the subcomponent list * test various methods of reading and changing the
        ...
        self.assertEqual(
            my_edt.get_subcircuit(sc).get_components(),
            ["C1", "X2", "L1"],
            "Subcircuit component list",
        )

        # START identical part with test_asc_editor.py:test_subcircuits_edit()
        self.assertEqual(
            my_edt.get_component_value(sc + ":L1"),
            "1µ",
            "Subcircuit Value for X1:L1, direct",
        )
        self.assertEqual(
            my_edt.get_subcircuit(sc).get_component_value("L1"),
            "1µ",
            "Subcircuit Value for X1:L1, indirect",
        )
        self.assertAlmostEqual(
            my_edt[sc + ":L1"].value,
            1e-6,
            msg="Subcircuit Value for X1:L1, float comparison",
        )

        my_edt.set_component_value(
            sc + ":L1", 2e-6
        )  # set float value, on compound component name
        self.assertEqual(
            my_edt[sc + ":L1"].value_str,
            "2u",
            "Subcircuit Value_str for X1:L1, after 1st change, direct",
        )
        self.assertEqual(
            my_edt.get_subcircuit(sc).get_component_value("L1"),
            "2u",
            "Subcircuit Value for X1:L1, after 1st change, indirect",
        )
        self.assertAlmostEqual(
            my_edt[sc + ":L1"].value,
            2e-6,
            msg="Subcircuit Value for X1:L1, after 1st change, float comparison",
        )
        ...
        self.assertEqual(
            my_edt[sc + ":L1"].value_str,
            "3µH",
            "Subcircuit Value_str for X1:L1, after 2nd change, direct",
        )
        self.assertEqual(
            my_edt.get_subcircuit(sc).get_component_value("L1"),
            "3µH",
            "Subcircuit Value for X1:L1, after 2nd change, indirect",
        )
        self.assertAlmostEqual(
            my_edt[sc + ":L1"].value,
            3e-6,
        ...
        self.assertEqual(
            my_edt[sc + ":L1"].value_str,
            "4uH",
            "Subcircuit Value_str for X1:L1, after 3rd change, direct",
        )
        self.assertEqual(
            my_edt.get_subcircuit(sc).get_component_value("L1"),
            "4uH",
            "Subcircuit Value for X1:L1, after 3rd change, indirect",
        )
        self.assertAlmostEqual(
            my_edt[sc + ":L1"].value,
            4e-6,
            msg="Subcircuit Value for X1:L1, after 3rd change, float comparison",
        )

        my_edt[sc + ":C1"].value = 22e-9
        self.assertEqual(
            my_edt[sc + ":C1"].value_str,
            "22n",
            "Subcircuit Value_str for X1:C1, after change",
        )
        self.assertAlmostEqual(
            my_edt.get_component_floatvalue(sc + ":C1"),
            22e-9,
            msg="Subcircuit Value for X1:C1, after change",
        )
        my_edt["R1"].value = 11
        ...
        my_edt.get_subcircuit(sc).set_component_parameters(
            "C1", Rser=1
        )  # set string value via indirect method
        self.assertEqual(
            my_edt.get_subcircuit(sc).get_component_parameters("C1"),
            {"Rser": 1},
            "Subcircuit parameters for X1:C1",
        )

        my_edt.save_netlist(temp_dir + "top_circuit_edit.net")
        self.equalFiles(
            temp_dir + "top_circuit_edit.net", golden_dir + "top_circuit_edit.net"
        )
        ...
        self.equalFiles(
            temp_dir + "top_circuit_edit1.net", golden_dir + "top_circuit_edit1.net"
        )
        my_edt[sc + ":X2:R1"].value = 99
        my_edt.save_netlist(temp_dir + "top_circuit_edit2.net")
        self.equalFiles(
            temp_dir + "top_circuit_edit2.net", golden_dir + "top_circuit_edit2.net"
        )
    ...
    def test_semiconductor_edits(self):
        ...
        self.equalFiles(
            golden_dir + "amp3_instance_edits.net", temp_dir + "amp3_instance_edits.net"
        )
        ...
        self.equalFiles(
            golden_dir + "amp3_subcircuit_edits.net",
            temp_dir + "amp3_subcircuit_edits.net",
        )
