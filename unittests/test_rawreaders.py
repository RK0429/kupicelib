#!/usr/bin/env python
# pyright: basic, reportUnknownMemberType=false, reportUnknownVariableType=false, reportAttributeAccessIssue=false, reportGeneralTypeIssues=false
...
class RawReader_Test(unittest.TestCase):
    ...
    def test_rawreaders_ac(self):
        for simulator in testset:
            dialect = None
            if (
                "force_dialect" in testset[simulator]
                and testset[simulator]["force_dialect"]
            ):
                dialect = simulator
            ...
            for file in testset[simulator]["ac"]["files"]:
                ...
                self.assertEqual(
                    raw.get_trace_names(),
                    testset[simulator]["ac"]["expected_tracenames"],
                    "Difference in trace names",
                )
                ...
                freqrange = (
                    int(abs(raw.get_trace("frequency").data[0])),
                    int(abs(raw.get_trace("frequency").data[tracelen - 1])),
                )
                self.assertEqual(
    ...
    def test_rawreaders_tran(self):
        for simulator in testset:
            ...
            for file in testset[simulator]["tran"]["files"]:
                ...
                self.assertEqual(
                    raw.get_trace_names(),
                    testset[simulator]["tran"]["expected_tracenames"],
                    "Difference in trace names",
                )
                ...
                if isinstance(expected_tracelen, int):
                    self.assertEqual(
                        tracelen, expected_tracelen, "Not the expected number of points"
                    )
                else:
                    self.assertEqual(
                        tracelen,
                        expected_tracelen[fileno],
                        "Not the expected number of points",
                    )
                ...
                timerange = (
                    abs(raw.get_trace("time").data[0]),
                    abs(raw.get_trace("time").data[tracelen - 1]),
                )
                self.assertEqual(
